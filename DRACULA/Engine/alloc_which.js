...
export function alloc_which(props)
{
    let {type, size, cluster, order, axis, copy} = props
    let source = []

    if (Number.isInteger(type)) {
        
        source = make_as_array(type, size)
...
function make_as_array(type, size)
{
    let array = []
    for(let item=0; item<size; item++)
    {
        array.push(parseInt(type))
    }
    return array
}
